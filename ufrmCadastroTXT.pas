...
begin
    ...
    cadastro.cpf := StrToFloat(edtCpf.Text);
  ...
  try
    if contLista>=1 then
      begin
        ...
        begin
          btnBack.Enabled:=false;
          Application.MessageBox('Inicio do cadastro atingido.','Inicio do Cadastro!',MB_OK+MB_ICONINFORMATION);
        end;
  except
    Application.MessageBox('Ops. Algum problema deve ser corrigido (index?).','Ops!',MB_OK+MB_ICONEXCLAMATION);
  end;
...
begin
  ...
  try
    ...
    Application.MessageBox('Nem um cadastro adicionado.','Cancelado!',MB_OK+MB_ICONINFORMATION);
  end;
  ...
  try
    if contLista<lista.Count-1 then
        begin
          ...
          begin
            btnNext.Enabled:=false;
            Application.MessageBox('Fim do cadastro atingido.','Fim do Cadastro!',MB_OK+MB_ICONINFORMATION);
            if lista.Count-1=0 then
              btnBack.Enabled:=false;
          end;
  except
...
begin
   //atribuindo o objeto do index ao cadastro
    ...
    begin
      btnCancel.Enabled:=false;
      Application.MessageBox('Nem um cadastro selecionado.','Não encontrado!',MB_OK+MB_ICONEXCLAMATION);
      btnDel.Enabled:=false;
    end
    else
      begin
...
begin
   //atribuindo o objeto do index ao cadastro
    ...
    begin
      ...
      edtCpf.Text := FloatToStr(cadastro.cpf);
      MostraIndex();
      btnDel.Enabled:=true;
    end
    else
